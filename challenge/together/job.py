...
train_fname = '../data/train_7.csv'
test_fname = '../data/test_7.csv'
...
f_con = ['cited_n','cited_age_min','cited_age_median','cited_age_max','cited_age_mean',
...
f_cat = ['APP_NB','APP_NB_PAYS','APP_NB_TYPE','VOIE_DEPOT','COUNTRY','SOURCE_BEGIN_MONTH','FISRT_APP_COUNTRY',
         ...
         'NB_BACKWARD','oecd_NB_ROOT_CLASSES','PRIORITY_MONTH','FILING_MONTH','PUBLICATION_MONTH','BEGIN_MONTH']
...
encoder = OneHotEncoder(categorical_features=range(len(f_con),len(f_con) + len(f_cat)), handle_unknown='ignore')
...
n_thread = -1
tree_per_forest = int(sys.argv[2])
n_leaf = int(sys.argv[3])
clf = RandomForestClassifier(n_estimators=tree_per_forest, max_leaf_nodes=n_leaf, oob_score=False, min_samples_leaf=1, min_samples_split=1, n_jobs=n_thread)
clf.fit(X_train,y_train)
y_pred_valid = clf.predict_proba(X_valid)[:, 1]

np.savetxt('../partial/'+sys.argv[2]+'_'+sys.argv[3]+'_'+sys.argv[1],y_pred_valid)
