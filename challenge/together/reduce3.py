...
#score = np.loadtxt('../bests/1')
#for i in range(2,int(sys.argv[1])+1):
#	if(i != 3 and i != 8):
#		score += np.loadtxt('../bests/'+str(i))

#np.savetxt('../bests/'+sys.argv[2]+'_'+sys.argv[3],np.argsort(-score)[:2000])
...
print('Score sur la validation : {}'.format(roc_auc_score(y_valid, y_pred_valid/(int(sys.argv[1])-2))))
